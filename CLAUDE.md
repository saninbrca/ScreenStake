...
27. **Group join = reserve-then-pay:** PI created only AFTER the transactional `joinReservations` slot reservation records it; every post-authorization join rejection cancels the PI before responding; the sweep cancels Stripe first, deletes the doc after; `startLockAt` blocks ONLY joining, TTL-bounded, and is OWNED by the run that stamped it — a start finding it fresh proceeds without overwriting and never releases it. (`docs/04`)
