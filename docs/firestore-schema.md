...
| `endDate` | number (millis) | Required | toMap (752) | `cancelOrRefundPayment` (index.ts:312); reconciliation `endDate<=now` query (index.ts:1688); `fetchActiveChallenges` (359) | **client UPDATE blocked**; numeric range query; tolerates Timestamp via `?.toMillis()` |
...
| `status` | string enum | Required | toMap (756) on create; `updateChallengeStatus` (FirestoreService.kt:219); CF settlement | many CFs + `fetchActiveChallenges`/`fetchFinishedChallenges` | `active` \| `completed` \| `failed`. **client UPDATE blocked** — CF owns active→completed/failed. (Client `updateChallengeStatus` exists but rules reject the protected `status` key.) |
...
| `failReason` | string enum | Added at LOSS settle | `checkPermissionViolations` (index.ts:1487, 1577); reconciliation (index.ts:1789, 1820) | — | values: `permission_violation` \| `usage_violation` \| `reconciliation` |
| `failedAt` | number (millis) | Added at LOSS settle | `checkPermissionViolations` (index.ts:1488, 1578); reconciliation (index.ts:1789, 1820) | — | `now` |
| `reconciliationLowEvidence` | boolean | Optional | reconciliation WIN low-evidence (index.ts:1893) | (admin review) | flags a refund made with zero nested dailyLogs |
...
`(mode ASC, status ASC, endDate ASC)` collection-group (indexes.json:52) — powers the reconciliation
due query. `createdAt` and `deviceId` single-field overrides (indexes.json:79, 88).
...
- **settle LOSS** — `checkPermissionViolations` / reconciliation set
  `status=failed, payoutStatus=captured, failReason, failedAt`.
- **delete** — client `deleteChallenge` (FirestoreService.kt:239) removes failed/cancelled docs so
  they don't re-sync; `deleteUserData` deletes all on account deletion. **Deleting fires
  `onChallengeDeleted` (index.ts:1997), which cascades a batched delete of the nested
...
| `reconciliationEnabled` | boolean | Optional | admin | `runDueChallengeReconciliation` (index.ts:1665) | **money-safety**: default false; reconciliation no-ops unless `=== true` |
| `reconciliationDryRun` | boolean | Optional | admin | `runDueChallengeReconciliation` (index.ts:1666) | default true; only an explicit `false` arms real Stripe ops |
...
- All **CF-written settlement fields**: `payoutStatus`, `payoutAmount`, `appFeeAmount`, `payoutDate`,
  `failReason`, `failedAt`, `reconciliationLowEvidence`. Room tracks the outcome only via `status`
...
> | **LOSS-device** | device-detected loss (limit exceeded) OR abandon | `DailyEvaluationWorker` capture→`updateChallengeStatus(FAILED)` (worker:564, VM:272) → repo:193 → **`deleteChallenge`** repo:195 → **doc DELETED** + `onChallengeDeleted` cascade (index.ts:1997) |
...
> | **ABANDON** | user abandons an active solo Hard Mode | `ActiveChallengeViewModel.abandonChallenge` VM:228 → capture → `markFailedAndFinish` → same **delete** path as LOSS-device |
...
> | **DELETE** | doc removed (cancel/fail cleanup or account deletion) | `deleteChallenge` FirestoreService.kt:234; `deleteUserData` FirestoreService.kt:697 |
>
> **The single most important asymmetry:** a **device-detected loss/abandon DELETES the whole
> challenge document** (and cascade-deletes its nested `dailyLogs`). It NEVER writes
> `failReason`/`failedAt`/`payoutStatus` to the doc — the doc simply ceases to exist. Only a
> **server-detected loss** (permission/usage/reconciliation) leaves a `status:"failed"` doc behind
> with those fields. Consequence: a `status:"failed"` solo challenge doc in Firestore was *always*
> settled server-side; device losses leave no doc at all.
...
| `status` | string `active\|completed\|failed` | CREATE (756, ="active") | WIN-server→`completed` (441); WIN-device-wrap→`completed`+syncedAt (FirestoreService.kt:219); LOSS-server→`failed` (1486/1574); reconciliation (1750/1789/1886). **LOSS-device/ABANDON ⇒ doc DELETED (no value change)** | client UPDATE of `status` blocked by rules — CF owns it |
...
| `payoutStatus` | string `refunded\|captured` | **ABSENT until settlement.** WIN-server→`refunded` (441); LOSS-server→`captured` (1489/1578); reconciliation (1789/1820/1887) | — | **never written on LOSS-device** (doc deleted instead) |
...
| `failReason` | string `permission_violation\|usage_violation\|reconciliation` | **ABSENT until LOSS-server.** (1487/1577, 1789/1820) | — | **never set on LOSS-device** (doc deleted) |
| `failedAt` | **number (millis)** | **ABSENT until LOSS-server.** (1488/1578, 1789/1820) | — | — |
| `reconciliationLowEvidence` | boolean | **ABSENT unless** a reconciliation WIN ran with zero nested dailyLogs (index.ts:1893) | — | admin-review flag |
...
LOSS — limit exceeded on a day, OR ABANDON (solo Hard Mode):   ⚠ DESTRUCTIVE
  ...
  2. updateChallengeStatus(FAILED) → ChallengeRepositoryImpl repo:193 → FirestoreService.deleteChallenge (repo:195)
       ⇒ the ENTIRE challenge doc is DELETED.
       ⇒ onChallengeDeleted trigger (index.ts:1997) batch-DELETES every nested dailyLogs/* doc.
     No failReason/failedAt/payoutStatus is ever stored — the doc is gone.
...
RECONCILIATION (server safety net, only if config flags armed; index.ts:1731+):
  ...
  • LOSS → same field delta as LOSS-server but failReason="reconciliation"

REDEEM-meta (on the FAILED ORIGINAL doc — only exists if that doc still exists, i.e. a
             server-side loss; a device-deleted loss has no doc to stamp):
...
DELETE (deleteChallenge on cancelled/failed cleanup, or deleteUserData on account deletion):
...
> (FirestoreService.kt:419). Because device losses delete the doc, the only `failed` docs it can
> ever return are server-side captures (permission/usage/reconciliation).
...
| **(whole doc)** | — | per-day create | **DELETED only by `onChallengeDeleted` cascade** (index.ts:1997) when the parent challenge is deleted — client delete blocked by rules | — |

> Timeline: `create` (clean day → `limitExceeded=false`) → repeated `merge` updates as usage
> accrues → possibly `false→true` on a violation day → cascade-`delete` if the parent challenge is
> deleted (device loss/abandon, cancel cleanup, account deletion). No standalone delete path.
...
| `reconciliationEnabled` | admin | **false** (index.ts:1665) | fail-SAFE: missing/unreadable ⇒ reconciliation disabled |
| `reconciliationDryRun` | admin | **true** (index.ts:1666) | only an explicit `false` arms real Stripe ops |
...
| challenge `failReason`,`failedAt` | a **server** loss (permission/usage/reconciliation) — never on device loss (doc deleted) |
...
| `failReason`,`failedAt` | ❌ | ✅ | server-loss only (permission/usage/reconciliation) — hard only |
...
| reconciliation (`scheduledChallengeReconciliation`) | ❌ | ✅ | due-query filters `mode=="hard"` (index.ts:1688) |
| `permissionStatus` CAPTURE (`capturedAt`/`captureReason`/`usageCapturedAt` + stake capture) | ❌ | ✅ | `checkPermissionViolations` skips non-hard (`cd["mode"] !== "hard"` index.ts:1479/1568). *(`permissionLostAt`/`usageViolationDetectedAt` markers are still WRITTEN device-side regardless of mode, but only acted on for hard.)* |
...
- **Fail:** a limit-exceeded day → `updateChallengeStatus(FAILED)` → **device delete path** deletes
  the Firestore doc + cascade dailyLogs (same destructive path as hard device-loss, but with NO
  money step). No `failReason`/`payoutStatus` is ever written.
