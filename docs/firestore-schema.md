...
| `groupWithdrawalWaivers.{groupId}` | map&lt;string, number&gt; | Optional | `GroupChallengeJoinViewModel.logWithdrawalWaiver` on confirmed join | — | legal proof: the JOINER's FAGG § 18 waiver, value = accept `Date.now()` in millis. Same dot-keyed-map shape as `pendingPayouts_completed`. Lives here, not on the group doc: the group doc's `withdrawalWaiverAccepted` is the CREATOR's single flag, and the participants array is CF-only (#28). Nested-map merge, so one group's entry never clobbers another's |
| `uninstallForfeitConsents.{challengeId}` | map&lt;string, number&gt; | Optional | `ChallengeCreationViewModel.recordUninstallForfeitConsent`, awaited BEFORE `createPaymentIntent` | — | legal proof: the SOLO creator's uninstall-forfeit consent (licenses the `device_dark` forfeit), value = the millis at which the box was actually ticked. Same dot-keyed-map shape as `groupWithdrawalWaivers`. Lives here, not on the challenge doc next to `withdrawalWaiverAccepted`: it must exist before the PaymentIntent, and pre-creating the challenge doc would burn the single rules-allowed CREATE (#3). Blocking — a failed write aborts before any Stripe call. Keyed by the unified `challengeId`, so an unpaid/cancelled attempt can leave an entry with no challenge |
...
| `bonusEnabled` | boolean | Required | client toMap, hardcoded `false` | `toGroupChallenge` (439) | **retired** — the wizard toggle is gone and no CF ever read it; still written so the doc shape (and the Room column) is unchanged. Dropping it buys a migration for no behavior change |
