...
27. **Group join = reserve-then-pay.** `joinGroupChallenge` creates the PaymentIntent ONLY after a transactional slot reservation (`joinReservations/{userId}`, 15-min TTL) that records where the PI lives; `confirmGroupJoin` converts reservation → participant transactionally and EVERY post-authorization rejection cancels the PI before responding; `sweepStaleJoinReservations` cancels Stripe FIRST and deletes the reservation doc only after a confirmed release. A stranded `startLockAt` may only ever block joining (TTL-bounded), never start/leave/settlement — and the stamp is OWNED by the run that wrote it: a start finding it already fresh PROCEEDS without overwriting (a fresh lock never blocks a start) and never releases it, because releasing a fence you do not own re-opens joining mid-capture and lands a paid joiner in an ACTIVE challenge with an uncaptured hold. A stale lock is still re-stamped and owned. → [docs/04](04_group_challenges.md)
28. **Group `participants` array writes = transactional merge.** Every CF mutation of the participants array runs a `runTransaction` that re-reads the doc and merges by `userId` — NEVER an in-memory full-array replacement (silently erases concurrent writes) and NEVER map-entry `arrayRemove`+`arrayUnion` (exact-match on heterogeneous entries silently no-ops, then duplicates). Stripe ops stay OUTSIDE the transactions. cancel/delete/expire fence joiners by stamping `startLockAt` and snapshotting in the SAME transaction; `completeGroupChallenge` stamps `settleLockAt` (CF-only field) the same way and computes ALL payout math from that snapshot; `failParticipant` refuses (409) while it is fresh. A stranded `settleLockAt` may only ever block `failParticipant`, TTL-bounded (15 min) — never join/leave/start/cancel or a settlement re-run after the TTL. A fail-vs-settle photo finish keeps the `failed` record and logs `FAIL-VS-SETTLE CONFLICT` (with groupId + userId) for manual review — that log is the resolution path, by decision. → [docs/04](04_group_challenges.md)
...
29. **Group stat doc: stale day stamp reads as ZERO, and ranking never gates money.** A daily value (`opensToday`/`timeUsedMinutes`/`exceededToday`) whose day stamp is not `DateUtils.todayKey()` reads as 0 — gated ONCE, in `GroupChallengeFirestoreService.withStats`, never re-implemented at a call site (the Room mirror cannot detect staleness on its own). Writes are lazy, so a clean day produces NO write and the doc keeps yesterday's numbers — the reset is impossible from the write side. The three writer paths (opens / time / exceeded) each own an INDEPENDENT day stamp; a shared stamp makes one writer's rollover silently drop another's pending day. Cumulative fields (`totalOpens`/`totalMinutes`/`exceededDays`) are the sum over days STRICTLY BEFORE their stamp, so "whole-challenge total" = `total + daily` UNGATED — which is what keeps `completeGroupChallenge` and every Stripe path out of the ranking feature. All ranking reads self-reported client counters and is COSMETIC: settlement classifies on `status` alone. → [docs/04](04_group_challenges.md)
