...
27. **Group join = reserve-then-pay.** `joinGroupChallenge` creates the PaymentIntent ONLY after a transactional slot reservation (`joinReservations/{userId}`, 15-min TTL) that records where the PI lives; `confirmGroupJoin` converts reservation → participant transactionally and EVERY post-authorization rejection cancels the PI before responding; `sweepStaleJoinReservations` cancels Stripe FIRST and deletes the reservation doc only after a confirmed release. A stranded `startLockAt` may only ever block joining (TTL-bounded), never start/leave/settlement. → [docs/04](04_group_challenges.md)
